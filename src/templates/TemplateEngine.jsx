export const generateTemplateHTML = (config) => {
  const { name, role, experiences, education, projects, templateId, fontFamily, baseFontSize, primaryColor } = config;

  // Inline Style untuk Font & Size
  const globalStyle = `font-family: '${fontFamily}', sans-serif; font-size: ${baseFontSize}px; line-height: 1.5;`;

  // Helper: Renderers
  const renderExp = () => experiences.map(exp => `
    <div class="mb-8">
      <div class="flex justify-between items-baseline mb-1">
        <h4 class="font-bold dark:text-white" style="font-size: 1.1em">${exp.company || 'Perusahaan'}</h4>
  ...
  const renderEdu = () => education.map(edu => `
    <div class="mb-4">
      <h4 class="font-bold dark:text-white" style="font-size: 1em">${edu.school || 'Sekolah'}</h4>
  ...
  const renderProj = () => projects.map(proj => `
    <div class="mb-6 p-5 bg-gray-50 dark:bg-gray-800 rounded-2xl border border-transparent hover:border-primary transition-all">
      <h4 class="font-bold text-primary mb-2" style="font-size: 1.1em">${proj.title || 'Proyek'}</h4>
      <p class="opacity-70 dark:text-gray-300 mb-3" style="font-size: 0.85em">${proj.desc || ''}</p>
      ${proj.link ? `<a href="${proj.link}" target="_blank" class="text-primary font-bold underline" style="font-size: 0.7em">VIEW 🔗</a>` : ''}
    </div>`).join('');
  ...
  switch (templateId) {
    case 'modern':
      return `
        <div style="${globalStyle}" class="p-8 md:p-20 bg-white dark:bg-gray-900 min-h-screen transition-colors duration-500">
    ...
    case 'creative':
      return `
        <div style="${globalStyle}" class="flex flex-col md:flex-row items-stretch min-h-screen bg-stone-100 dark:bg-zinc-950 transition-colors duration-500">
    ...
    case 'classic':
      return `
        <div style="${globalStyle}" class="p-6 md:p-10 bg-gray-100 dark:bg-slate-950 min-h-screen transition-colors duration-500 flex justify-center">
          <div class="max-w-4xl w-full bg-white dark:bg-slate-900 shadow-2xl p-8 md:p-16 border-double border-4 border-primary transition-colors duration-500">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-12 pt-10 border-t dark:border-slate-800">
              <section><h3 class="font-bold mb-6 dark:text-white" style="font-size: 1em">EDUCATION</h3>${renderEdu()}</section>
              <section><h3 class="font-bold mb-6 dark:text-white" style="font-size: 1em">PROJECTS</h3>${renderProj()}</section>
            </div>
          </div>
        </div>`;
    ...
    case 'terminal':
      return `
        <div style="${globalStyle}" class="p-4 md:p-10 bg-[#0d0d0d] text-[#00ff41] min-h-screen">
