export const generateTemplateHTML = (config) => {
  const { experiences, education, projects, name, role, primaryColor, templateId } = config;

  // Helper: Render List Pengalaman
  const expHTML = experiences.map(exp => `
    <div class="mb-8">
      <div class="flex justify-between items-baseline mb-1">
        <h4 class="font-bold dark:text-white" style="font-size: 1.1em">${exp.company || 'Nama Perusahaan'}</h4>
        <span class="opacity-50 font-mono" style="font-size: 0.7em">${exp.dates || 'Tahun'}</span>
      </div>
      <div class="text-primary font-semibold mb-2" style="font-size: 0.9em">${exp.role || 'Jabatan'}</div>
      <p class="leading-relaxed opacity-70 dark:text-gray-300" style="font-size: 0.9em">${exp.desc || ''}</p>
    </div>`).join('');

  // Helper: Render List Proyek
  const projHTML = projects.map(proj => `
    <div class="mb-6 p-5 bg-gray-50 dark:bg-gray-800 rounded-2xl border border-transparent hover:border-primary transition-all">
      <div class="flex justify-between items-start mb-2">
        <h4 class="font-bold text-primary" style="font-size: 1.1em">${proj.title || 'Judul Proyek'}</h4>
        ${proj.link ? `<a href="${proj.link}" target="_blank" class="opacity-50 hover:opacity-100">🔗</a>` : ''}
      </div>
      <p class="opacity-70 dark:text-gray-300 mb-2" style="font-size: 0.85em">${proj.desc || ''}</p>
    </div>`).join('');

  // Helper: Render List Pendidikan
  const eduHTML = education.map(edu => `
    <div class="mb-4">
      <h4 class="font-bold dark:text-white" style="font-size: 1em">${edu.school || 'Nama Sekolah'}</h4>
    ...
    return `
      <div class="p-8 md:p-20 bg-white dark:bg-gray-900 min-h-screen transition-colors duration-500">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-16">
          ...
          <section>
            <h3 class="font-black opacity-30 uppercase mb-8 tracking-widest dark:text-white" style="font-size: 0.7em">Featured Projects</h3>
            ${projHTML}
          </section>
        </div>
      </div>`;
      ...
      <div class="flex flex-col md:flex-row items-stretch min-h-screen bg-stone-100 dark:bg-zinc-950 transition-colors duration-500">
        ...
        <div class="w-full md:w-2/3 p-8 md:p-12 space-y-10 overflow-y-auto">
          <div class="bg-white dark:bg-zinc-900 p-10 rounded-[2.5rem] shadow-sm">
            <h2 class="font-black uppercase mb-8 flex items-center gap-2 dark:text-white" style="font-size: 1.2em"><span class="w-8 h-1 bg-primary"></span> JOURNEY</h2>
            ${expHTML}
          </div>
          <div class="bg-white dark:bg-zinc-900 p-10 rounded-[2.5rem] shadow-sm">
            <h2 class="font-black uppercase mb-8 flex items-center gap-2 dark:text-white" style="font-size: 1.2em"><span class="w-8 h-1 bg-primary"></span> PROJECTS</h2>
            <div class="grid grid-cols-1 gap-4">${projHTML}</div>
          </div>
          <div class="bg-white dark:bg-zinc-900 p-10 rounded-[2.5rem] shadow-sm">
            <h2 class="font-black uppercase mb-8 flex items-center gap-2 dark:text-white" style="font-size: 1.2em"><span class="w-8 h-1 bg-primary"></span> EDUCATION</h2>
            ${eduHTML}
          </div>
        </div>
      </div>`;
      ...
      <div class="p-6 md:p-10 bg-gray-100 dark:bg-slate-950 min-h-screen font-serif transition-colors duration-500 flex justify-center">
        <div class="max-w-4xl w-full bg-white dark:bg-slate-900 shadow-2xl p-8 md:p-16 border-double border-4 border-primary transition-colors duration-500">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-12 pt-10 border-t dark:border-slate-800">
            <section><h3 class="font-bold mb-6 dark:text-white">EDUCATION</h3>${eduHTML}</section>
            <section><h3 class="font-bold mb-6 dark:text-white">PROJECTS</h3>${projHTML}</section>
          </div>
        </div>
      </div>`;
  }
