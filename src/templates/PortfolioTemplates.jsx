  ...
  <div className="min-h-full bg-[#f4f4f4] dark:bg-slate-950 p-6 md:p-10 font-serif">
    <div className="max-w-4xl mx-auto bg-white dark:bg-slate-900 shadow-2xl p-8 md:p-16 border-t-[12px]" style={{ borderColor: config.primaryColor }}>
      <div className="text-center border-b pb-10 mb-10 dark:border-slate-800">
        <h1 className="font-bold mb-2 uppercase tracking-tight" style={{ fontSize: '2.25em' }}>
          {config.name}
        </h1>
        <p className="text-gray-500 italic" style={{ fontSize: '1.125em' }}>
          {config.role}
        </p>
      </div>
  ...
  <div className="min-h-full bg-yellow-50 dark:bg-zinc-950 p-4">
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 h-full">
      <div className="md:col-span-1 p-8 rounded-3xl text-white flex flex-col justify-end shadow-lg" style={{ backgroundColor: config.primaryColor }}>
        <h1 className="font-black leading-none mb-4" style={{ fontSize: '2.25em' }}>{config.name}</h1>
        <p className="font-medium opacity-80" style={{ fontSize: '1.125em' }}>{config.role}</p>
      </div>
      ...
      <div className="md:col-span-2 space-y-4">
          ...
          <div className="space-y-6">
            {config.experiences.map(exp => (
              <div key={exp.id} className="relative pl-6 border-l-2 border-dashed border-gray-200 dark:border-zinc-700">
                ...
                <h4 className="font-bold" style={{ fontSize: '1em' }}>{exp.company}</h4>
        ...
        <div className="bg-white dark:bg-zinc-900 p-8 rounded-3xl shadow-sm">
          ...
          {config.education.map(edu => (
            <div key={edu.id} className="mb-4">
              <h4 className="font-bold" style={{ fontSize: '1em' }}>{edu.school}</h4>
    ...
    <div className="max-w-3xl mx-auto border border-[#333] rounded-lg overflow-hidden shadow-[0_0_30px_rgba(0,0,0,0.5)]">
      ...
      <div className="p-6 md:p-10 space-y-8 bg-black/90">
        <header>
          ...
          <h1 className="font-bold uppercase" style={{ fontSize: '1.875em' }}>{`> ${config.name}`}</h1>
          <p className="text-white" style={{ fontSize: '1em' }}>{config.role}</p>
        </header>
        ...
        <section>
          ...
          {config.experiences.map(exp => (
            <div key={exp.id} className="mb-6 border-l border-[#00ff00]/30 pl-4">
              ...
              <p style={{ fontSize: '0.75em' }}>Role: {exp.role} [{exp.dates}]</p>
              <p className="opacity-70 leading-relaxed text-gray-400" style={{ fontSize: '0.75em' }}>{`// ${exp.desc}`}</p>
            </div>
          ))}
        </section>
        ...
        <section>
          ...
          {config.education.map(edu => (
            <p key={edu.id} style={{ fontSize: '0.875em' }} className="mb-2">{`>> ${edu.school} -- ${edu.degree}`}</p>
          ))}
        </section>
      </div>
    </div>
