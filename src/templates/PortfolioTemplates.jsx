...
export const CreativeTemplate = ({ config }) => (
  <div className="h-full min-h-[85vh] bg-stone-100 dark:bg-zinc-950 flex flex-col md:flex-row items-stretch transition-colors duration-500">
    <div className="w-full md:w-1/3 p-10 flex flex-col justify-end text-white relative overflow-hidden" style={{ backgroundColor: config.primaryColor }}>
    ...
    <div className="w-full md:w-2/3 p-6 md:p-12 space-y-10 overflow-y-auto">
      ...
      <div className="bg-white dark:bg-zinc-900 p-8 md:p-10 rounded-[2.5rem] shadow-sm">
        ...
        {config.projects.map(proj => (
          <div key={proj.id} className="p-6 border-2 border-dashed rounded-3xl mb-4 dark:border-zinc-800" style={{ borderColor: `${config.primaryColor}33` }}>
            <h4 className="font-black mb-2 uppercase dark:text-white" style={{ fontSize: '1em' }}>{proj.title}</h4>
            <p className="opacity-60 dark:text-gray-400" style={{ fontSize: '0.85em' }}>{proj.desc}</p>
          </div>
        ))}
      </div>
    </div>
        ...
        <div className="space-y-12">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <section>
                    ...
                    {config.education.map(edu => (
                        <div key={edu.id} className="mb-4 text-sm opacity-80">{`{ "${edu.school}": "${edu.degree}" }`}</div>
                    ))}
                </section>
            </div>
        </div>
